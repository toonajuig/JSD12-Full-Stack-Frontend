...
const BASE = 'http://localhost:3002/api/v2/users/pg'

function SupabasePanel() {
  const [postForm, setPostForm] = useState({ username: '', email: '', password: '', role: 'user' })
  const [putForm, setPutForm] = useState({ id: '', username: '', role: 'user' })
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-lg border border-gray-200 p-5">
        <h2 className="mb-3 font-semibold text-gray-700">GET /users/pg</h2>
        <button
          onClick={handleGet}
      ...
      {/* POST */}
      <div className="rounded-lg border border-gray-200 p-5">
        <h2 className="mb-4 font-semibold text-gray-700">POST /users/pg</h2>
        <form onSubmit={handlePost} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={loading.post} className="rounded bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-600 disabled:opacity-50">
            {loading.post ? 'Sending...' : 'POST /users/pg'}
          </button>
        </form>
      ...
      {/* PUT */}
      <div className="rounded-lg border border-gray-200 p-5">
        <h2 className="mb-4 font-semibold text-gray-700">PUT /users/pg/:id</h2>
        <form onSubmit={handlePut} className="flex flex-col gap-3">
          <input className="input" placeholder="UUID" value={putForm.id} onChange={e => setPutForm(p => ({ ...p, id: e.target.value }))} />
          <input className="input" placeholder="username" value={putForm.username} onChange={e => setPutForm(p => ({ ...p, username: e.target.value }))} />
          <select className="input" value={putForm.role} onChange={e => setPutForm(p => ({ ...p, role: e.target.value }))}>
            <option value="user">user</option>
            <option value="admin">admin</option>
          </select>
          <button type="submit" disabled={loading.put} className="rounded bg-yellow-500 px-4 py-2 text-sm font-medium text-white hover:bg-yellow-600 disabled:opacity-50">
            {loading.put ? 'Sending...' : 'PUT /users/pg/:id'}
          </button>
        </form>
      ...
      {/* DELETE */}
      <div className="rounded-lg border border-gray-200 p-5">
        <h2 className="mb-4 font-semibold text-gray-700">DELETE /users/pg/:id</h2>
        <form onSubmit={handleDelete} className="flex flex-col gap-3">
          <input className="input" placeholder="UUID" value={deleteId} onChange={e => setDeleteId(e.target.value)} />
          <button type="submit" disabled={loading.delete} className="rounded bg-red-500 px-4 py-2 text-sm font-medium text-white hover:bg-red-600 disabled:opacity-50">
            {loading.delete ? 'Sending...' : 'DELETE /users/pg/:id'}
          </button>
        </form>
